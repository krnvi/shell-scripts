...
export INP=/home/OldData/windpowerFcst/input/

#export CURDATE=`date -d "-2 day" +%Y%m%d`
...
cd $MAIN/namescripts/
$MAIN/namescripts/change_namelist ${STARTDATE} 24

cat namelist.wps-test.bot >>namelist.wps
cat namelist.input-test.bot >>namelist.input
...
./link_grib.csh $INP/$STARTDATE/*.grb2
...
#/root/it/skymet_mail_sender.pl vineethpk@skymet.net,anil.kumar@skymetweather.com,basanta.samal@skymetweather.com "06GMT Power Model started:`hostname`"
...
#/bin/mail -s "Model failed  Power 06GMT: $FINDUSER" $EMAIL_ID < /dev/null
  exit
fi
