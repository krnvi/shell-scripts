#!/bin/bash
export power=/home/Data/WRF-NMM18/NMMV3.2.1/WPPV3/rrun/poutpost
...
export date=$1
tmdat=`date --d="${date:0:8} +1day" +%Y%m%d%H`
mkdir -p $outFiles/$tmdat
cd $outFiles/$tmdat/

ln -sf $scripts/anaikadavu77.lst masterlist
ln -sf $power/$date/wrfpost$date.ctl wrfpost$date.ctl
ln -sf $power/$date/wrfpost$date.idx wrfpost$date.idx
ln -sf $power/$date/wrfpost$date wrfpost$date
...
cat masterlist | while IFS=, read ID STNNAME LAT LON SLP INTR; do
export fileName=$STNNAME"_"$tmdat.csv
...
if (wspd>3&wspd<5)
'define p1=(57.6*'wspd')-156.4'
else
'define p1=1513.09/pow((1+exp(9.64-1.01*'wspd')),(1/2.01))'
endif
...
'!echo 'ti','hh':'mi','ws1','wdir','power' >>' dat.csv
tt=tt+1
mi=mi+15
p1=0
endwhile
